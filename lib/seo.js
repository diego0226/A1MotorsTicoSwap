...
/**
 * Un proyecto del taller: el antes y el después de un swap.
 *
 * Va como `ImageGallery` porque el contenido real de la página son las fotos.
 * Cada una se declara con su `contentUrl` y su descripción, que es lo que
 * necesita Google Imágenes para indexarlas por separado y mandar tráfico a la
 * ficha en vez de solo al dominio.
 */
export function projectSchema({ project, path }) {
  const url = absoluteUrl(path);
  // El después va de primero: la foto [0] se marca como representativa de la
  // página y conviene que sea el resultado, no el motor viejo.
  const photos = [
    ...(project.after ?? []),
    ...(project.during ?? []),
    ...(project.before ?? []),
  ...
  return {
    ...
    about: {
      '@type': 'Service',
      name: `Swap de motor ${project.from} a ${project.to} en ${project.vehicle}`,
      serviceType: 'Conversión de motor V8',
      provider: { '@id': ORGANIZATION_ID },
      areaServed: { '@type': 'Country', name: 'Costa Rica' },
    },
...
/** Índice de proyectos, para que Google entienda /proyectos como una colección. */
export function projectListSchema({ path, projects }) {
  const url = absoluteUrl(path);
  ...
  return {
    ...
    '@type': 'CollectionPage',
    '@id': `${url}#coleccion`,
    name: 'Proyectos de swap LS y V8',
    description:
      'Trabajos de conversión de motor terminados en A1 Motors Tico Swap, con el antes y el después de cada vehículo.',
    url,
    inLanguage: site.lang,
    publisher: { '@id': ORGANIZATION_ID },
    mainEntity: {
